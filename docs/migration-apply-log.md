...
| 0050 (`meet_and_greet_and_pets_reach`, Phase 4) | 2026-07-08 | Applied wrapped in `begin / commit` (commit `91e7d79`). **Applied BEFORE 0049** (out of filename order — independent; 0049 awaits the 2b deploy runbook). **All checks passed.** **(1)** `messages.kind` present — `text NOT NULL DEFAULT 'text'`; backfill count of non-`text` rows = 0. **(2) guard_message_update** — a `kind` flip on soft-delete rejected with `'messages.kind is immutable'`; a plain soft-delete (no kind touch) still succeeds (0044 semantics intact). **(3) pets policy** — `pg_policies` shows a SINGLE `pets_select_owner_or_booking_host` SELECT policy (no parallel duplicate). **(4) INSERT kind-vs-role impersonation suite (D-A1 TIGHT):** starter→`meet_greet_request` PASS; host→`meet_greet_confirmed` PASS; **FORGED** starter→`meet_greet_confirmed` **rejected by RLS** (R2 verified); `kind='text'` unchanged. **(5) pets reach (Layer 2 root fix, live):** on multi-pet repro `494087eb` the host now sees **2 of 2** pets (was 1 pre-fix). Client build (MG helper, CTAs, pills + divider, i18n) greenlit — Strategy reviews on push. |
...
`pg_publication_tables` check confirmed the publication had been EMPTY
since birth; see the toggle's confirmed row.)*
