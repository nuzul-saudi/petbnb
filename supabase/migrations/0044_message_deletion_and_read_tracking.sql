...
-- 2026-06-28 — confirmed via pg_constraint query on the live DB
-- that the 0001 auto-named check is exactly `messages_body_check`
-- with definition `CHECK ((length(body) > 0))`. Explicit drop
-- preferred over the earlier defensive find-by-definition DO
-- block (cleared by Strategy review).
alter table public.messages
  drop constraint messages_body_check;

-- Replacement check, Strategy's preferred form:
-- a LIVE message (deleted_at IS NULL) requires non-null,
-- non-empty body. A DELETED message (deleted_at IS NOT NULL)
-- has no body constraint (the soft-delete UPDATE nulls it).
alter table public.messages
  add constraint messages_body_presence check (
    (deleted_at is null and body is not null and length(body) > 0)
...
--   expect: includes 'messages_body_presence' with the new
--           conditional definition. The 0001 'messages_body_check'
--           is gone (replaced).
--
